...
def gen_forcing_functions(y_des, dt=.001, alpha=10, beta=10/4., num_samples=10):
    ...
    # you lose important features. Too many and the derivatives shrink to
    ...
    for d in range(y_des.shape[0]):
        ...
        attrac_force = ddy_des[d] - alpha * (beta * (goal[d] - y_des[d]) - dy_des[d])
        # generate another interpolation function we can use
        # to now train up our decoded oscillator output
        # the forcing function is not usually the cause of distortion
        forcing_functions.append(lambda x, inter_force=attrac_force:
                                 interpolate.interp1d(np.linspace(-np.pi, np.pi, num_samples), inter_force)(x))
        #plt.plot(forcing_functions[-1](np.linspace(-np.pi, np.pi, num_samples)))
        #plt.show()

    return forcing_functions


def gen_point_attractor(model, in_goal, n_neurons=200, alpha=10, beta=10/4., seed=0):
...
def force_theta(spk, dec, function, gain=1):
    return function(np.dot(spk, dec[:])) * gain
