...
def scalar_blend(sig_a, sig_b, seed=0):
    with nengo.Network() as dd_model:
        ...
        output = nengo.Node(lambda t, x: x[0]*x[1], size_in=2)
        ...
        conn_a = nengo.Connection(prod_a, output[0], function=lambda x: x[0]*x[1])
        conn_b = nengo.Connection(prod_b, output[1], function=lambda x: x[0]*x[1])
