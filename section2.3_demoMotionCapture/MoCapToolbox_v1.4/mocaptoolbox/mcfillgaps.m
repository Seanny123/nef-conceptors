...
if nargin==2
    ...
    elseif strcmp(maxfill,'nobefill') % mcfillgaps(d, 'string'); if no fill of beginning and end - BB 20100917
        maxfill=1000000;
        method='nobefill';
    elseif isletter(maxfill)
        s1='Sorry, do not understand "';
        s2='". Please check you spelling.';
        disp(strcat(s1,maxfill,s2));
        disp('No gap fill performed.');
        %method='';
        d2=d;
        return
    else method=''; %mcfillgaps(d,1000);
    end
end
...
elseif isfield(d,'type') && strcmp(d.type, 'segm data')
    ...
    disp('This function works only with MoCap, norm, and segm structures.');
end
