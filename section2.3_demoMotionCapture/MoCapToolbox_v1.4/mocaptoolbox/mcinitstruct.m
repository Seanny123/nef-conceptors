...
% d1 = mcinitmocap;
% d1 = mcinitmocap(type);
% d1 = mcinitmocap(type, data, freq);
% d1 = mcinitmocap(type, data, freq, markerName);
% d1 = mcinitmocap(type, data, freq, markerName, fn);
% d1 = mcinitmocap(data, freq);
% d1 = mcinitmocap(data, freq, markerName);
% d1 = mcinitmocap(data, freq, markerName, fn);
%
% input parameters
...
% examples
% d1 = mcinitmocap;
% d1 = mcinitmocap('norm data', data);
% d1 = mcinitmocap(data, 120, markernames, 'mydata1.tsv');
% 
% comments
...
if nargin==2
    if isnumeric(type) && isnumeric(data) %data and freq given
        freq=data;
        data=type;
        type='MoCap data';
        markerName={};
