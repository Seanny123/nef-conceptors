function d2 = mcj2sHJ(d, par)
%%% Difference to original: 
%%% angles in d2.rootrot.az are given in rad, not degrees.
...
if isfield(d,'type') && strcmp(d.type, 'MoCap data') && isfield(par,'type') && strcmp(par.type, 'j2spar')
    ...
    frontalPlane = cross(d.data(:,ind2)-d.data(:,ind1),...
        d.data(:,ind3)-d.data(:,ind1), 2);
    norm = mcnorm(frontalPlane);
    frontalPlaneUnit = frontalPlane ./ repmat(norm,1,3);
    [th,phi,r] = ...
        cart2sph(frontalPlaneUnit(:,1),frontalPlaneUnit(:,2),frontalPlaneUnit(:,3));
    d2.rootrot.az = unwrap(th);
    ...
    d = mcrotate(d, - 180 * d2.rootrot.az / pi); %%%% added 101208 PT
    ...
    for k = 1:length(par.parent)
        if par.parent(k)>0
            ...
            % Quaternions
            %d2.segm(k).quat = smoothquat(dir2quatHJ(d2.segm(k).eucl));

            d2.segm(k).quat = (dir2quatHJ(d2.segm(k).eucl));
