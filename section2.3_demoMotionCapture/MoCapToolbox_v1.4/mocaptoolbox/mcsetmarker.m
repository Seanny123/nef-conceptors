...
if length(mnum) ~= d1.nMarkers %FIX BB20130210
    fprintf('\nError: Replacement mocap structure d1 must have the same amount \nof markers as indicated in mnum. Data has not been replaced.\n')
    return
end

if isfield(d,'type') && strcmp(d.type, 'MoCap data')   
    if min(mnum)<1 || max(mnum)>d.nMarkers
        fprintf('\nNote: Marker number higher than in original data structure.\n')
    end   
