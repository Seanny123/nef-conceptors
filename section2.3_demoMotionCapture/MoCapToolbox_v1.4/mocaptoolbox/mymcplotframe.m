function par = mymcplotframe(d, n, p, proj)
%%%%%% from mcplotframe
%%%%%% addition: animpar structure has field p.fignr for the figure number
%%%%%% addition: p.fontsize
%%%%%% different location of fig
...
for k=1:size(x,1) % main loop
    ...
    set(gcf,'Position',[900 500 p.scrsize(1) p.scrsize(2)]) ; % DVD: w=720 h=420
...
%                plot(x(k,m),z(k,m),[mcol(m) 'o'],'MarkerSize',p.msize(min(m,length(p.msize))),'MarkerFaceColor',mcol(m))
    ...
    if p.showfnum
        h=text(minxx+0.95*(maxxx-minxx), minzz+0.97*(maxzz-minzz), num2str(k),...
            'HorizontalAlignment','Right','FontSize',p.fontsize,'FontWeight','bold');
