...
%%
if 0
mcplottimeseries(d, 10, 'dim', 3, 'timetype', 'frame');
end 
%%
...
m2jpar.markerNum{1,12} = [26 27 28 29]; % head LBHD RBHD LFHD RFHD
m2jpar.markerNum{1,11} = [35 24]; % neck CLAV C7
m2jpar.markerNum{1,13} = [22]; % right shoulder RSHO
m2jpar.markerNum{1,17} = [23]; % left shoulder LSHO
m2jpar.markerNum{1,14} = [14]; % right elbow RELB
m2jpar.markerNum{1,18} = [15]; % left elbow LELB
m2jpar.markerNum{1,15} = [16 6]; % right wrist RWRB RWRA
m2jpar.markerNum{1,19} = [5 11]; % left wrist LWRA LWRB
m2jpar.markerNum{1,16} = [13]; % right hand RFIN
m2jpar.markerNum{1,20} = [10]; % left hand LFIN
m2jpar.markerNum{1,10} = [17 9]; % back center T10 STRN
m2jpar.markerNum{1,1} = [2 1 4 3]; % root (back bottom) 
                                       % RFWT LFWT RBWT LBWT
m2jpar.markerNum{1,2} = [2 4]; % right hip RFWT RBWT
m2jpar.markerNum{1,6} = [1 3]; % left hip LFWT LBWT
m2jpar.markerNum{1,3} = [36]; % right knee RKNE
m2jpar.markerNum{1,7} = [19]; % left knee LKNE
m2jpar.markerNum{1,4} = [39 40]; % right ankle  RHEE RANK
m2jpar.markerNum{1,8} = [41 30]; % left ankle LHEE LANK
m2jpar.markerNum{1,5} = [33 34]; % right foot RMT5 RTOE
m2jpar.markerNum{1,9} = [32 31]; % left foot LMT5 LTOE
...
[nnRawDataCartWheel, hmeanCartWheel, segLengthsCartWheel] = ...
...
if 0
freq = 120; 
djBack =  nnRaw2jHJ_absH(nnRawDataCartWheel, hmeanCartWheel, ...
     segLengthsCartWheel, j2spar, freq);
...
if 0
    figure(1); clf;
    plot(nnRawDataCartWheel(:,10:12));
end
refsig = nnRawDataCartWheel(:,11);
...
if 1
    plCartWheel = plOpt1;
    save nnRawCartWheel nnRawDataCartWheel hmeanCartWheel ...        
        plCartWheel segLengthsCartWheel;
end
