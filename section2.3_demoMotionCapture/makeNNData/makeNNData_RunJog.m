...
m2jpar.markerNum{1,12} = [13 12 26 14]; % head LBHD RBHD LFHD RFHD
m2jpar.markerNum{1,11} = [35 7]; % neck CLAV C7
m2jpar.markerNum{1,13} = [4]; % right shoulder RSHO
m2jpar.markerNum{1,17} = [8]; % left shoulder LSHO
m2jpar.markerNum{1,14} = [16]; % right elbow RELB
m2jpar.markerNum{1,18} = [22]; % left elbow LELB
m2jpar.markerNum{1,15} = [11 5]; % right wrist RWRB RWRA
m2jpar.markerNum{1,19} = [20 15]; % left wrist LWRA LWRB
m2jpar.markerNum{1,16} = [9]; % right hand RFIN
m2jpar.markerNum{1,20} = [23]; % left hand LFIN
m2jpar.markerNum{1,10} = [1 33]; % back center T10 STRN
m2jpar.markerNum{1,1} = [24 19 6 18]; % root (back bottom) 
                                       % RFWT LFWT RBWT LBWT
m2jpar.markerNum{1,2} = [24 6]; % right hip RFWT RBWT
m2jpar.markerNum{1,6} = [19 18]; % left hip LFWT LBWT
m2jpar.markerNum{1,3} = [32]; % right knee RKNE
m2jpar.markerNum{1,7} = [28]; % left knee LKNE
m2jpar.markerNum{1,4} = [38 36]; % right ankle  RHEE RANK
m2jpar.markerNum{1,8} = [30 40]; % left ankle LHEE LANK
m2jpar.markerNum{1,5} = [37 39]; % right foot RMT5 RTOE
m2jpar.markerNum{1,9} = [41 29]; % left foot LMT5 LTOE
...
[nnRawDataRunJog, hmeanRunJog, segLengthsRunJog] = j2nnRawHJ_absH(dj,j2spar);
...
if 0
freq = 120; 
djBack =  nnRaw2jHJ_absH(nnRawDataRunJog, hmeanRunJog, ...
     segLengthsRunJog, j2spar, freq);
...
if 0
    figure(1); clf;
    plot(nnRawDataRunJog(:,10:12));
end
%%
refsig = nnRawDataRunJog(:,11);
...
sineRunJog = sin(2 * pi * (args + sOpt1) / plOpt1);
sineRunJog2 = sin(2 * pi * (args + sOpt1 + 2 * plOpt1/3) / plOpt1);
sineRunJog3 = sin(2 * pi * (args + sOpt1 + 4 * plOpt1/3) / plOpt1);
sineRunJog4 = sin(2 * pi * (args + sOpt1 + 4 * plOpt1/2) / plOpt1);
...
if 1
    plRunJog = plOpt1;
    save nnRawRunJog nnRawDataRunJog hmeanRunJog ...        
        plRunJog segLengthsRunJog;
end
