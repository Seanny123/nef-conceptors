...
m2jpar.markerNum{1,12} = [4 10 7 30]; % head LBHD RBHD LFHD RFHD
m2jpar.markerNum{1,11} = [11 15]; % neck CLAV C7
m2jpar.markerNum{1,13} = [5]; % right shoulder RSHO
m2jpar.markerNum{1,17} = [6]; % left shoulder LSHO
m2jpar.markerNum{1,14} = [3]; % right elbow RELB
m2jpar.markerNum{1,18} = [17]; % left elbow LELB
m2jpar.markerNum{1,15} = [32 1]; % right wrist RWRB RWRA
m2jpar.markerNum{1,19} = [14 25]; % left wrist LWRA LWRB
m2jpar.markerNum{1,16} = [33]; % right hand RFIN
m2jpar.markerNum{1,20} = [28]; % left hand LFIN
m2jpar.markerNum{1,10} = [20 24]; % back center T10 STRN
m2jpar.markerNum{1,1} = [22 23 16 27]; % root (back bottom) 
                                       % RFWT LFWT RBWT LBWT
m2jpar.markerNum{1,2} = [22 16]; % right hip RFWT RBWT
m2jpar.markerNum{1,6} = [23 27]; % left hip LFWT LBWT
m2jpar.markerNum{1,3} = [21]; % right knee RKNE
m2jpar.markerNum{1,7} = [2]; % left knee LKNE
m2jpar.markerNum{1,4} = [36 35]; % right ankle  RHEE RANK
m2jpar.markerNum{1,8} = [41 37]; % left ankle LHEE LANK
m2jpar.markerNum{1,5} = [26 34]; % right foot RMT5 RTOE
m2jpar.markerNum{1,9} = [39 38]; % left foot LMT5 LTOE
...
[nnRawDataSlowWalk, hmeanSlowWalk, segLengthsSlowWalk] = ...
    j2nnRawHJ_absH(dj,j2spar);

%% backtransform to joint data for test and show video
if 0
freq = 120; 
djBack =  nnRaw2jHJ_absH(nnRawDataSlowWalk, hmeanSlowWalk, ...
     segLengthsSlowWalk, j2spar, freq);
...
if 0
    figure(1); clf;
    plot(nnRawDataSlowWalk(:,10:12));
end
refsig = nnRawDataSlowWalk(:,11);
...
sineSlowWalk = sin(2 * pi * (args + sOpt1) / plOpt1);
sineSlowWalk2 = sin(2 * pi * (args + sOpt1 + 2 * plOpt1/3) / plOpt1);
sineSlowWalk3 = sin(2 * pi * (args + sOpt1 + 4 * plOpt1/3) / plOpt1);
sineSlowWalk4 = sin(2 * pi * (args + sOpt1 + 4 * plOpt1/2) / plOpt1);
if 0
    figure(1); clf;
    plot(refsig, 'r'); hold on;  
    plot(sin(2 * pi * (args + sOpt) / plOpt));
    plot(sineSlowWalk, 'g');
    plot(sineSlowWalk2, 'c');
     plot(sineSlowWalk3, 'm');
end
...
if 1
    plSlowWalk = plOpt1;
    save nnRawSlowWalk nnRawDataSlowWalk hmeanSlowWalk ...
        plSlowWalk segLengthsSlowWalk;
end
